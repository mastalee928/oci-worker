...
public class AliDNSService {
    ...
    public String testAccountConfig(String accessKeyId, String accessKeySecret) {
        try {
            ...
            if (json.containsKey("Code")) {
                throw new OciException("Aliyun DNS test failed: " + json.getStr("Message"));
            }
            return "Connection successful";
        } catch (Exception e) {
            throw new OciException("Aliyun DNS test failed: " + e.getMessage());
        }
    }
    ...
    public List<Map<String, Object>> listDomains(int page, int perPage) {
        if (!isConfigured()) throw new OciException("Aliyun DNS not configured");
        try {
            Map<String, String> params = new LinkedHashMap<>();
            params.put("Action", "DescribeDomains");
            params.put("Version", "2015-01-09");
            ...
            JSONObject json = JSONUtil.parseObj(resp.body());
            if (json.containsKey("Code")) {
                throw new OciException(json.getStr("Message", "Query failed"));
            }
            ...
            throw new OciException("Query domains failed: " + e.getMessage());
        }
    }
}
