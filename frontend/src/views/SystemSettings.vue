      ...
      <a-tab-pane key="notify" tab="消息通知">
        <a-card title="Telegram 通知" class="settings-card-wide">
          ...
          <a-form v-else layout="vertical">
            ...
            <a-form-item label="Webhook 路径密钥（可选）">
              <a-input-password v-model:value="tgConfig.webhookPathSecret" placeholder="一般留空即可：点「一键同步」时会自动生成" />
              <div style="margin-top: 6px; font-size: 12px; color: var(--text-sub)">
                仅在服务器已配置 TG_WEBHOOK_SECRET 且与库中不一致时需手填；否则留空最省事。
              </div>
            </a-form-item>
            ...
            <a-form-item label="Webhook 公网根地址">
              ...
              <div style="margin-top: 6px; font-size: 12px; color: var(--text-sub)">
                填好公网地址后点「一键同步」即可（路径密钥可留空，由后台自动生成并保存）。
              </div>
            </a-form-item>
            <a-space wrap>
...
async function applyTgWebhookNow() {
  ...
  try {
    ...
    const d = res.data || {}
    let tip = `已同步。allowedUpdates: ${(d.allowedUpdates || []).join(', ') || '—'}；pending: ${d.pendingUpdateCount ?? '—'}`
    if (d.pathSecretAutoGenerated) {
      tip += '。已自动生成并保存 Webhook 路径密钥（无需手填）'
    }
    message.success(tip)
