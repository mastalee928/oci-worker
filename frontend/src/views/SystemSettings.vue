        ...
        <a-card title="Telegram 通知" class="settings-card-wide">
          ...
          <a-form v-else layout="vertical">
            ...
            <a-form-item label="Webhook 公网根地址">
              ...
              <div style="margin-top: 6px; font-size: 12px; color: var(--text-sub)">
                填好地址后点「一键同步」；路径密钥由后台自动生成并保存，无需填写。
              </div>
            </a-form-item>
            <a-space wrap>
...
async function applyTgWebhookNow() {
  ...
  const raw = tgWebhookPublicUrl.value?.trim()
  if (!raw) {
  ...
  if (!u.toLowerCase().startsWith('https://')) {
    message.warning('公网地址须以 https:// 开头（Telegram 要求）。仅 IP:端口 时请先用隧道域名。')
    return
  }
