<template>
  <section class="protection-panel">
    ...
    <div v-if="configVisible" class="overlay" role="presentation">
      <section class="dialog" role="dialog" aria-modal="true" aria-labelledby="quota-dialog-title">
        ...
        <div class="dialog-body">
          ...
          <div class="section"><div class="section-title"><i>CPU</i><strong>计算实例</strong></div><div class="rows">
            <div v-for="key in computeKeys" :key="key" class="row"><div class="name"><strong>{{ resource(key).label }}</strong><span>{{ descriptions[key] }}</span></div><div class="control"><input v-model.number="form.values[key]" type="range" :min="resource(key).min" :max="resource(key).max" :step="resource(key).step" :disabled="!form.enabled[key]" @input="form.profile='CUSTOM'"><span class="value">{{ form.values[key] }} {{ resource(key).unit }}</span></div><button v-if="form.profile==='CUSTOM'" :class="['switch',{on:form.enabled[key]}]" type="button" @click="setEnabled(key,!form.enabled[key])"></button><span v-else class="state">{{ form.enabled[key]?'已限制':'不限制' }}</span></div>
    ...
    <div v-if="verifyVisible" class="overlay" role="presentation"><section class="dialog sm" role="dialog" aria-modal="true"><header class="dialog-head"><div><h3>{{ verifyTitle }}</h3><p>验证码已发送至Telegram，有效期5分钟。</p></div><button class="x" type="button" @click="verifyVisible=false">×</button></header><div class="dialog-body"><input v-model="verifyCode" class="tg-input" maxlength="6" inputmode="numeric" placeholder="请输入6位验证码" @keyup.enter="submitVerified"></div><footer class="dialog-foot"><span class="foot">高风险写操作需要二次确认。</span><div class="actions"><button class="demo-btn" type="button" @click="verifyVisible=false">取消</button><button class="demo-btn primary" type="button" :disabled="submitting" @click="submitVerified">确认{{ pendingAction==='disable'?'关闭':'启用' }}</button></div></footer></section></div>
  </section>
</template>
...
function setEnabled(key:string,value:boolean){form.enabled[key]=value}
