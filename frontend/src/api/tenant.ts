...
const protectionHandledErrorConfig: OciRequestConfig = { skipBusinessMessage: true, skipErrorMessage: true }
...
export function getAuditLogs(data: { id: string; days?: number; domainId?: string; mode?: 'login' | 'audit' }) {
...
export function unlockOrganizationAction(data: { id: string; action: 'createChildTenancy' | 'inviteOrganizationTenancy'; verifyCode: string }) { return request.post('/oci/user/organization/unlock', data, organizationHandledErrorConfig) }
...
export function markAnnouncementRead(data: { id: string; announcementId: string }) {
  return request.post('/oci/user/announcement/read', data)
}
